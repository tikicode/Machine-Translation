...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.n_layers = 1
        self.forwardPass = True

        self.embedding = nn.Embedding(input_size, hidden_size)
        self.ForwardLSTM = BiDirectionalLSTM(self.input_size, self.hidden_size)
        self.BackwardLSTM = BiDirectionalLSTM(self.input_size, self.hidden_size)
        # self.LSTM2 = BiDirectionalLSTM(self.input_size, self.hidden_size)
    ...
    def forward(self, input, hidden):
        ...
        if self.forwardPass:
            forward_output, forward_hidden = self.ForwardLSTM.forward(input, hidden)
            return forward_output, forward_hidden
        else:
            backward_output, backward_hidden = self.ForwardLSTM.forward(input, hidden)
            self.forwardPass = not self.forwardPass
            return backward_output, backward_hidden
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        if hidden[0].dim() == 2:
            ...
            attn_applied = torch.matmul(attn_weights,    encoder_outputs).unsqueeze(0)
            ...
            output = F.relu(output)
            output, hidden = self.LSTM.forward(output, hidden[0])
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    # REMEMBER WHEN DONE TO DO ENCODER AND DECODER
    ...
    decoder_hidden = encoder_hidden[0].resize(1,1,256)
    ...
    for di in range(target_length):
        ...
        maxidx=torch.where(decoder_output[0]==max(decoder_output[0]))[0]
        decoder_input = maxidx.squeeze().detach()

        loss += criterion(decoder_output, target_tensor[di])
        if np.array(decoder_input) == "<EOS>":
            break
...
def main():
    ...
    optimizer = optim.SGD(params, lr=args.initial_learning_rate)
