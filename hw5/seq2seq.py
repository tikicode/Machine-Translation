...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.lstm = nn.GRU(hidden_size, hidden_size) # since hidden batch is 0
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    decoder.train()
    optimizer.zero_grad()
    e_out, e_hidden = encoder(input_tensor, encoder_hidden)
    print(e_hidden[1].shape, e_hidden[0].shape, e_out.shape)

    loss = 0

    for idx in range(target_tensor.shape[0]):
        decoder_output, decoder_hidden, _ = decoder(decoder_input, e_hidden, e_out)
